...
#define mAppTaskPrio_c_LEDS  5
#define mAppStackSize_c_LEDS 500

#define tmrTimeInSeconds_counter 2
...
void App_TransmitUartData_ctr_t (uint8_t counter_timer);
void App_init( void );
...
uint16_t appTimeoutMs_counter = 2000;
...
OSA_TASK_DEFINE( AppLEDS, mAppTaskPrio_c_LEDS, 1, mAppStackSize_c_LEDS, FALSE );
...
osaEventId_t          mAppEvent_Switch;
osaTaskId_t           mAppHandler_LEDS;
...
void App_init( void )
{
    ...
    mAppEvent_Switch = OSA_EventCreate(TRUE);
    ...
    mTimer_c_counter = TMR_AllocateTimer();
                ...
                {   
                    ...
                    {          
                        ...
                        { 
                            ...
                            TMR_StartIntervalTimer(mTimer_c_counter, appTimeoutMs_counter, counter_increase, NULL); // start timer
                            App_TransmitUartData_ctr_t(counter_timer); // send the initial value from the counter=0;

                            mAppHandler_LEDS = OSA_TaskCreate(OSA_TASK(AppLEDS), NULL); // initialization of our task to change leds color
...
void counter_increase (void *pData)
{
	...
	if(counter_timer < 5){
		counter_timer++;
		App_TransmitUartData_ctr_t(counter_timer);
	}
	else{
		counter_timer = 0;
		App_TransmitUartData_ctr_t(counter_timer);
...
void AppLEDS (void *pData)
{
	...
	{
		TurnOffLeds();
		if (counter_timer == 0) //red
			Led2On();
		else if (counter_timer == 1) //green
			Led3On();
		else if (counter_timer == 2) //blue
			Led4On();
		else if (counter_timer == 3) //cian
		{
			Led3On();
			Led4On();
		}
		else if (counter_timer == 4) //purple
		{
			Led2On();
			Led4On();
		}
		else if (counter_timer == 5) //all leds turn on
			TurnOnLeds();
	}
}
  ...
  )
{
    ...
    case gKBD_EventSW4_c:
    	if (events == 1 && gState != stateInit ) // use to avoid that in the initialization change the value of the counter
    		{
    	    	counter_timer = 1;
    	    	TMR_StopTimer( mTimer_c_counter );
    	    	TMR_StartIntervalTimer(mTimer_c_counter, appTimeoutMs_counter, counter_increase, NULL);
    	    	App_TransmitUartData_ctr_t(counter_timer);
    	    }
    	    else if (events == 2 && gState != stateInit )
    	    	{
    	    		counter_timer = 3;
    	    		TMR_StopTimer( mTimer_c_counter );
    	    		TMR_StartIntervalTimer(mTimer_c_counter, appTimeoutMs_counter, counter_increase, NULL);
